    ...
    <section class="bg-gradient-to-br from-emerald-500 to-emerald-700 text-white py-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <p class="text-xl text-emerald-100 max-w-3xl mx-auto fade-in">
                Pilih program yang sesuai dengan kebutuhan belajar Anda. Kami menyediakan berbagai program untuk semua
                jenjang pendidikan.
            </p>
        </div>
    </section>
    ...
    <section class="bg-white py-8 shadow-md sticky top-20 z-40">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-wrap justify-center gap-3">
                <button onclick="filterProgram('all')"
                    class="filter-btn active px-4 py-1.5 rounded-lg font-medium text-sm transition duration-300">
                    Semua Program
                </button>
                <button onclick="filterProgram('sd')"
                    class="filter-btn px-4 py-1.5 rounded-lg font-medium text-sm transition duration-300">
                    SD
                </button>
                <button onclick="filterProgram('smp')"
                    class="filter-btn px-4 py-1.5 rounded-lg font-medium text-sm transition duration-300">
                    SMP
                </button>
                <button onclick="filterProgram('sma')"
                    class="filter-btn px-4 py-1.5 rounded-lg font-medium text-sm transition duration-300">
                    SMA
                </button>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="programs-container">
                @forelse($programs as $program)
                    <div class="program-card bg-white rounded-xl shadow-md hover:shadow-xl transition duration-300"
                        ...
                        @else
                            <div
                                class="h-48 bg-gradient-to-br from-{{ $program->category == 'sd' ? 'blue' : ($program->category == 'smp' ? 'purple' : 'orange') }}-400 to-{{ $program->category == 'sd' ? 'blue' : ($program->category == 'smp' ? 'purple' : 'orange') }}-600 flex items-center justify-center rounded-t-xl">
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between mb-3">
                                <span
                                    class="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-sm font-semibold uppercase">
                                    {{ $program->category }}
                                </span>
                            ...
                            <div class="description-container mb-4">
                                @if(strlen($program->description) > 100)
                                    ...
                                    <button type="button"
                                        class="toggle-description text-emerald-600 hover:text-emerald-700 text-sm font-medium mt-1 focus:outline-none"
                                        onclick="toggleDescription(this)">
                                        Selengkapnya <svg class="w-4 h-4 inline-block transition-transform duration-200 arrow-icon"
                                    ...
                                    <ul class="space-y-2">
                                        @foreach($program->subjects as $subject)
                                            <li class="flex items-center text-gray-700 text-sm">
                                                <svg class="w-4 h-4 text-emerald-500 mr-2 flex-shrink-0" fill="currentColor"
                                                    viewBox="0 0 20 20">
                            ...
                            <div class="border-t pt-4 mt-4">
                                ...
                                <a href="{{ route('pendaftaran', ['program' => $program->title]) }}"
                                    class="block w-full bg-emerald-500 text-white text-center px-6 py-3 rounded-lg font-semibold hover:bg-emerald-600 transition duration-300">
                                    Daftar Sekarang
                                </a>
                            </div>
                        </div>
    ...
    <section class="py-16 bg-white">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="https://wa.me/6282237343764?text=Halo,%20saya%20ingin%20konsultasi%20tentang%20program%20bimbel"
                    target="_blank"
