...
@section('content')
    ...
    <!-- Hero Section -->
    <section class="bg-white pt-20 pb-16 lg:pt-24 lg:pb-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
                <!-- Left Content -->
                <div class="fade-in">
                    <h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 leading-tight mb-6">
                        {{ $heroSection->title ?? 'Belajar, Berkembang, Dan Berkarya Di Desa Gajah' }}
                    </h1>
                    <p class="text-base text-gray-700 leading-relaxed mb-8">
                        {{ $heroSection->subtitle ?? 'Taman Belajar Sedjati adalah bimbel pelajaran utama yang ditujukan untuk para murid agar dapat belajar lebih fokus dan mendapatkan pendampingan dari guru secara privat. Tombol utama mengarah ke pendaftaran & jadwal hari ini' }}
                    </p>
                    <div class="flex flex-col sm:flex-row gap-4">
                        <a href="{{ $heroSection->cta_link ?? route('pendaftaran') }}"
                            class="inline-block bg-emerald-500 text-white px-8 py-4 rounded-lg font-semibold text-center hover:bg-emerald-600 transition duration-300 shadow-lg hover:shadow-xl">
                            {{ $heroSection->cta_text ?? 'Daftar Sekarang' }}
                        </a>
                        <a href="{{ route('program') }}"
                            class="inline-block bg-white text-gray-800 px-8 py-4 rounded-lg font-semibold text-center hover:bg-gray-50 transition duration-300 shadow-md hover:shadow-lg border-2 border-gray-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @forelse($programs as $program)
                    <div class="bg-white rounded-xl shadow-md hover:shadow-xl transition duration-300 overflow-hidden">
                        ...
                        @else
                            <div
                                class="h-48 bg-gradient-to-br from-{{ $program->category == 'sd' ? 'emerald' : ($program->category == 'smp' ? 'blue' : 'purple') }}-400 to-{{ $program->category == 'sd' ? 'emerald' : ($program->category == 'smp' ? 'blue' : 'purple') }}-600 flex items-center justify-center">
                        ...
                        <div class="p-6">
                                ...
                                <ul class="space-y-2 mb-6">
                                    @foreach(array_slice($program->subjects, 0, 3) as $subject)
                                        <li class="flex items-center text-gray-700">
                                            <svg class="w-5 h-5 text-emerald-500 mr-2 flex-shrink-0" fill="currentColor"
                                                viewBox="0 0 20 20">
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('pendaftaran', ['program' => $program->title]) }}"
                                    class="flex-1 text-center bg-emerald-500 text-white px-4 py-3 rounded-lg font-semibold hover:bg-emerald-600 transition duration-300">
                                    Daftar
                                </a>
                                <a href="{{ route('program') }}"
                                    class="flex-1 text-center bg-white text-emerald-600 border border-emerald-500 px-4 py-3 rounded-lg font-semibold hover:bg-emerald-50 transition duration-300">
                                    Detail
                                </a>
                            </div>
                        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @forelse($testimonials as $testimonial)
                    <div
                        class="bg-gray-50 p-6 rounded-xl shadow-md hover:shadow-lg transition duration-300 transform hover:-translate-y-1 h-full flex flex-col">
                        <div class="flex items-center mb-4">
                            <div class="w-12 h-12 flex-shrink-0">
                                @if($testimonial->image_url)
                                    <img src="{{ $testimonial->image_url }}" alt="{{ $testimonial->name }}"
                                        class="w-full h-full object-cover rounded-full border-2 border-emerald-400">
                                @else
                                    <div
                                        class="w-full h-full bg-emerald-500 rounded-full flex items-center justify-center text-white font-bold text-xl">
                                        {{ substr($testimonial->name, 0, 1) }}
                                    </div>
                                @endif
                            </div>
                            <div class="ml-4">
                                <h4 class="font-semibold text-gray-900 line-clamp-1" title="{{ $testimonial->name }}">
                                    {{ $testimonial->name }}
                                </h4>
                                <p class="text-sm text-gray-600 line-clamp-1" title="{{ $testimonial->role }}">
                                    {{ $testimonial->role }}
                                </p>
                            </div>
                        </div>
                ...
                @empty
                    <div class="col-span-3 text-center py-8">
                        <p class="text-gray-500 italic">Belum ada testimoni.</p>
                    </div>
                @endforelse
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                @forelse($statistics as $statistic)
                    <div class="transform hover:scale-105 transition duration-300">
                        <div class="text-4xl md:text-5xl font-bold mb-2">{{ $statistic->value }}</div>
                        <div class="text-emerald-100 text-lg">{{ $statistic->label }}</div>
                    </div>
                @empty
