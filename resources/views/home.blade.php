                ...
                <div class="bg-white rounded-xl shadow-md hover:shadow-xl transition duration-300 overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex gap-2">
                            <a href="{{ route('pendaftaran', ['program' => $program->title]) }}" class="flex-1 text-center bg-emerald-500 text-white px-4 py-3 rounded-lg font-semibold hover:bg-emerald-600 transition duration-300">
                                Daftar
                            </a>
                            <a href="{{ route('program') }}" class="flex-1 text-center bg-white text-emerald-600 border border-emerald-500 px-4 py-3 rounded-lg font-semibold hover:bg-emerald-50 transition duration-300">
                                Detail
                            </a>
                        </div>
                    </div>
...
<section class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="bg-gray-50 p-6 rounded-xl shadow-md hover:shadow-lg transition duration-300 transform hover:-translate-y-1 h-full flex flex-col">
                <div class="flex items-center mb-4">
                    <div class="w-12 h-12 flex-shrink-0">
                        @if($testimonial->image_url)
                            <img src="{{ $testimonial->image_url }}" alt="{{ $testimonial->name }}" class="w-full h-full object-cover rounded-full border-2 border-emerald-400">
                        @else
                            <div class="w-full h-full bg-emerald-500 rounded-full flex items-center justify-center text-white font-bold text-xl">
                                {{ substr($testimonial->name, 0, 1) }}
                            </div>
                        @endif
                    </div>
                    <div class="ml-4">
                        <h4 class="font-semibold text-gray-900 line-clamp-1" title="{{ $testimonial->name }}">{{ $testimonial->name }}</h4>
                        <p class="text-sm text-gray-600 line-clamp-1" title="{{ $testimonial->role }}">{{ $testimonial->role }}</p>
                    </div>
                </div>
